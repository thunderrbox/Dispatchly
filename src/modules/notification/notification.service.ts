...
export async function sendStatusChangeEmail(
  ...
  const subject = `[Daffodil] Shipment ${orderId.substring(0, 8)} Status Update: ${newStatus}`;
  
  const htmlContent = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
      <h2 style="color: #4f46e5; margin-bottom: 20px;">Daffodil Delivery Tracker</h2>
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: 'Daffodil Tracker <onboarding@resend.dev>', // Resend free tier sandbox domain
      to: customerEmail,
      subject: subject,
      html: htmlContent,
    });
