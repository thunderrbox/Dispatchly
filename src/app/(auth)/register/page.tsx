...
import { User, Mail, Lock, ShieldCheck, ArrowRight, Sparkles, KeyRound, AtSign, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div suppressHydrationWarning className="relative min-h-screen flex items-center justify-center bg-[#0B0A0F] text-[#FAF7F2] p-4 py-8 overflow-hidden font-sans">
      ...
      {!isSuccess ? (
        ...
        /* Animated Success Modal Screen */
        <motion.div
          initial={{ opacity: 0, scale: 0.85, y: 15 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
          className="relative z-10 w-full max-w-sm bg-[#161520]/90 backdrop-blur-2xl border border-white/15 p-8 rounded-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] text-center space-y-5"
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            transition={{ type: 'spring', stiffness: 260, damping: 20, delay: 0.1 }}
            className="w-16 h-16 bg-[#10B981] rounded-full mx-auto flex items-center justify-center shadow-lg shadow-emerald-500/30"
          >
            <CheckCircle2 className="w-10 h-10 text-white" />
          </motion.div>
          
          <div className="space-y-1.5">
            <h2 className="text-2xl font-extrabold text-white tracking-tight">Success!</h2>
            <p className="text-xs text-white/70">Successfully logged into <span className="font-bold text-white">Dispatchly</span></p>
          </div>

          <div className="pt-2 flex items-center justify-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
            <p className="text-[11px] text-white/40 font-mono tracking-wider">Redirecting in 2s...</p>
          </div>
        </motion.div>
      )}
